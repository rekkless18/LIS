import React, { useEffect } from 'react';
import { Card, Pagination, Row, Col } from 'antd';
import { MsExceptionSearchPanel } from '@/components/test/MsExceptionSearchPanel';
import { MsExceptionActionBar } from '@/components/test/MsExceptionActionBar';
import { MsExceptionLeftTable } from '@/components/test/MsExceptionLeftTable';
import { MsExceptionRightTable } from '@/components/test/MsExceptionRightTable';
import { useMsExceptionStore } from '@/stores/msException';
const MsException: React.FC = () => {
  const { query, pagination, setPagination, resetFilters } = useMsExceptionStore();
  useEffect(() => { query(); }, []);
  const handleSearch = () => { setPagination({ current: 1 }); query(); };
  const handleReset = () => { resetFilters(); setPagination({ current: 1 }); query(); };
  const onPageChange = (page: number, pageSize?: number) => { setPagination({ current: page, pageSize: pageSize || pagination.pageSize }); query(); };
  return (
    <div className="p-4">
      <MsExceptionSearchPanel onSearch={handleSearch} onReset={handleReset} />
      <MsExceptionActionBar onQuery={handleSearch} onReset={handleReset} />
      <Row gutter={16}>
        <Col span={12}>
          <Card title="质谱异常任务" size="small" styles={{ body: { padding: 16 } }}>
            <MsExceptionLeftTable />
          </Card>
        </Col>
        <Col span={12}>
          <Card title="异常结果与历史记录" size="small" styles={{ body: { padding: 16 } }}>
            <MsExceptionRightTable />
          </Card>
        </Col>
      </Row>
      <div className="py-4">
        <Pagination current={pagination.current} pageSize={pagination.pageSize} total={pagination.total} showSizeChanger={pagination.showSizeChanger} pageSizeOptions={pagination.pageSizeOptions} onChange={onPageChange} showTotal={(total) => (<span>共 {total} 条，第 {pagination.current} 页</span>)} size="small" />
      </div>
    </div>
  );
};
export default MsException;
