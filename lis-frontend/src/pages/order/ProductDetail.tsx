...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      const foundOrder = orders.find(o => o.id === id && o.type === 'product');
      if (foundOrder) {
        setOrder(foundOrder);
  ...
  return (
    <div className="h-full bg-white p-6 overflow-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-semibold">查看订单详情</h1>
      ...
      <Form layout="vertical">
        ...
        <Card title="样本信息" className="mb-6">
          <Row gutter={16}>
            <Col xs={24} sm={12} md={6}>
              <Form.Item label="样本编号">
                <Input value={order.sampleNos[0] || ''} disabled />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={6}>
              <Form.Item label="原始样本编号">
                <Input value={order.originalSampleNos?.[0] || ''} disabled />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={6}>
              <Form.Item label="样本类型">
                <Input value={order.sampleTypes[0] || ''} disabled />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={6}>
          ...
          <Row gutter={16}>
            <Col xs={24} sm={12} md={12}>
              <Form.Item label="产品名称">
                <Input value={order.items.map((item: any) => item.product.name).join(', ')} disabled />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={12}>
              <Form.Item label="交付要求">
                <Input value={order.deliveryRequirements.join(', ')} disabled />
              </Form.Item>
            </Col>
          </Row>
        </Card>
