  ...
  useEffect(() => {
    if (id) {
      ...
      if (foundOrder) {
        ...
        setSelectedProducts(foundOrder.items.map((item: any) => item.product));
        ...
        form.setFieldsValue({
          ...
          sampleNo: foundOrder.sampleNos[0] || '',
          originalSampleNo: foundOrder.originalSampleNos[0] || '',
          sampleType: foundOrder.sampleTypes[0] || '',
          samplingTime: null,
          products: foundOrder.items.map((item: any) => item.product.name).join(', '),
        ...
        <Card title="样本信息" className="mb-6">
          <Row gutter={16}>
            <Col xs={24} sm={12} md={6}>
              ...
              >
                <Input placeholder="请输入样本编号" disabled />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12} md={6}>
