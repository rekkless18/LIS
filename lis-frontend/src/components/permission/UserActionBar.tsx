...
export const UserActionBar: React.FC<Props> = ({ onQuery, onReset }) => {
  ...
  const handleCreateSubmit = () => {
    form.validateFields().then(values => {
      createItem({ account: values.account, name: values.name, userType: values.userType as UserType, status: values.status as EnableStatus, createdAt: new Date().toISOString() })
  ...
  const handleOpenEdit = () => {
    ...
    form.setFieldsValue({ account: target.account, name: target.name, userType: target.userType, status: target.status, roles: target.roles, email: target.email, phone: target.phone })
    setOpenEdit(true)
  }
  const handleEditSubmit = () => {
    const id = selectedRowKeys[0] as string
    form.validateFields().then(values => {
      editItem(id, { account: values.account, name: values.name, userType: values.userType as UserType, status: values.status as EnableStatus, roles: values.roles || [], email: values.email, phone: values.phone })
      setOpenEdit(false)
  ...
  const handleOpenRoles = () => {
    ...
    const target = filteredItems.find(d => d.id === selectedRowKeys[0])
    if (!target) { message.warning('未找到选中用户'); return }
    form.setFieldsValue({ roles: target.roles })
    loadRolesOptions()
  ...
  return (
    <div className="flex justify-between items-center py-2">
      ...
      <Modal title="新建用户" open={openCreate} onOk={handleCreateSubmit} onCancel={() => setOpenCreate(false)} okText="提交" cancelText="关闭">
      ...
      <Modal title="编辑用户" open={openEdit} onOk={handleEditSubmit} onCancel={() => setOpenEdit(false)} okText="提交" cancelText="关闭">
        <Form form={form} layout="vertical">
          <Form.Item label="用户账号" name="account" rules={[{ required: true, message: '请输入用户账号' }]}><Input /></Form.Item>
          <Form.Item label="用户姓名" name="name" rules={[{ required: true, message: '请输入用户姓名' }]}><Input /></Form.Item>
          <Form.Item label="联系方式" name="phone" rules={[{ required: true, message: '请输入联系方式' }]}><Input /></Form.Item>
