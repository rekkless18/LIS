...
export const SpecialBioinfoTable: React.FC = () => {
  const { filteredTasks, selectedRowKeys, setSelectedRowKeys } = useSpecialBioinfoStore();
  ...
  const columns: ColumnsType<any> = [
    ...
    { title: '分析状态', dataIndex: 'status', key: 'status', width: 120, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-20">{text}</span></Tooltip>) },
    { title: '审核状态', dataIndex: 'auditStatus', key: 'auditStatus', width: 120, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-20">{text}</span></Tooltip>) },
    { title: '分析设备/流水线', dataIndex: 'pipeline', key: 'pipeline', width: 200, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-40">{text}</span></Tooltip>) },
