...
export const SpecialExceptionLeftTable: React.FC = () => {
  const { filteredTasks, selectedRowKeys, setSelectedRowKeys, setActiveTaskId } = useSpecialExceptionStore();
  ...
  const columns: ColumnsType<any> = [
    ...
    { title: '实验设备', dataIndex: 'device', key: 'device', width: 160 },
    ...
    { title: '重测时间', dataIndex: 'retestTime', key: 'retestTime', width: 160, render: (text: string) => { if (!text) return ''; const d = new Date(text); const s = d.toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }); return (<Tooltip title={s}><span className="truncate block max-w-40">{s}</span></Tooltip>); } }
  ];
