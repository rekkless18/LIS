...
import { useMsAuditStore, MsAuditStatus } from '@/stores/msAudit';
...
export const MsAuditSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  const [form] = Form.useForm();
  const { filters, setFilters } = useMsAuditStore();
  const [collapsed, setCollapsed] = useState(true);

  const statusOptions: { value: MsAuditStatus; label: string }[] = ['待审核','已审核','不通过'].map(s => ({ value: s as MsAuditStatus, label: s }));
