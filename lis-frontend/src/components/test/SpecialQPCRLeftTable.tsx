...
export const SpecialQPCRLeftTable: React.FC = () => {
  const { filteredTasks, selectedRowKeys, setSelectedRowKeys, setActiveTaskId } = useSpecialQPCRStore();
  ...
  const columns: ColumnsType<any> = [
    ...
    { title: '分析状态', dataIndex: 'analysisStatus', key: 'analysisStatus', width: 120 },
  ...
  return (
    <Table
      ...
      onRow={(record) => ({ onClick: () => setActiveTaskId(record.id) })}
    />
  );
};
