...
import { useSpecialExceptionStore, SpecialExceptionStatus } from '@/stores/specialException';
...
export const SpecialExceptionSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  const [form] = Form.useForm();
  const { filters, setFilters } = useSpecialExceptionStore();
  const [collapsed, setCollapsed] = useState(true);

  const statusOptions: { value: SpecialExceptionStatus; label: string }[] = ['待处理','已重测','已提交'].map(s => ({ value: s as SpecialExceptionStatus, label: s }));
