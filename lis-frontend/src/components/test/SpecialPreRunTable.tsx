...
export const SpecialPreRunTable: React.FC = () => {
  const { filteredTasks, selectedRowKeys, setSelectedRowKeys } = useSpecialPreRunStore();
  ...
  const columns: ColumnsType<any> = [
    ...
    { title: '处理状态', dataIndex: 'status', key: 'status', width: 120, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-20">{text}</span></Tooltip>) },
    { title: '质控结果', dataIndex: 'qcResult', key: 'qcResult', width: 160, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-40">{text || ''}</span></Tooltip>) },
    { title: '设备/工位', dataIndex: 'workstation', key: 'workstation', width: 160, render: (text: string) => (<Tooltip title={text}><span className="truncate block max-w-40">{text || ''}</span></Tooltip>) },
  ...
  return (
    <Table rowKey="id" rowSelection={rowSelection} columns={columns} dataSource={filteredTasks} scroll={{ x: totalWidth }} pagination={false} className="cursor-pointer" />
  );
};
