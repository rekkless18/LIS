...
import { useSpecialQPCRStore, QPCRAnalysisStatus, QPCRAuditStatus } from '@/stores/specialQPCR';
...
export const SpecialQPCRSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  const [form] = Form.useForm();
  const { filters, setFilters } = useSpecialQPCRStore();
  const [collapsed, setCollapsed] = useState(true);

  const analysisOptions: { value: QPCRAnalysisStatus; label: string }[] = ['待分析','进行中','已完成','失败'].map(s => ({ value: s as QPCRAnalysisStatus, label: s }));
  const auditOptions: { value: QPCRAuditStatus; label: string }[] = ['待审核','已审核','不通过'].map(s => ({ value: s as QPCRAuditStatus, label: s }));

  useEffect(() => {
    form.setFieldsValue({
      sampleNos: filters.sampleNos?.join(', ') || '',
      productIds: filters.productIds?.join(', ') || '',
      analysisStatuses: filters.analysisStatuses || analysisOptions.map(s => s.value),
      auditStatuses: filters.auditStatuses || auditOptions.map(s => s.value)
    });
  }, [filters, form]);
  ...
  const handleSearch = () => {
    form.validateFields().then(values => {
      const next = {
        ...
        analysisStatuses: values.analysisStatuses || undefined,
        auditStatuses: values.auditStatuses || undefined,
    ...
    <Card title="查询条件" size="small" styles={{ body: { padding: 16 } }} extra={<Button type="link" icon={collapsed ? <DownOutlined /> : <UpOutlined />} onClick={() => setCollapsed(!collapsed)}>{collapsed ? '展开' : '收起'}</Button>}>
      <Form form={form} layout="vertical">
        <Row gutter={16}>
          ...
          <Col xs={24} sm={12} md={6} lg={6} xl={6}><Form.Item label="分析状态" name="analysisStatuses"><Select mode="multiple" placeholder="请选择分析状态">{analysisOptions.map(s => (<Option key={s.value} value={s.value}>{s.label}</Option>))}</Select></Form.Item></Col>
        ...
        {!collapsed && (<><Row gutter={16}><Col xs={24} sm={12} md={6} lg={6} xl={6}><Form.Item label="开始时间" name="startRange"><RangePicker showTime={{ format: 'HH:mm:ss' }} format="YYYY-MM-DD HH:mm:ss" placeholder={["开始时间","结束时间"]} /></Form.Item></Col><Col xs={24} sm={12} md={6} lg={6} xl={6}><Form.Item label="完成时间" name="endRange"><RangePicker showTime={{ format: 'HH:mm:ss' }} format="YYYY-MM-DD HH:mm:ss" placeholder={["开始时间","结束时间"]} /></Form.Item></Col></Row></>)}
      </Form>
    </Card>
  );
};
