...
import { useSpecialPreRunStore, PreRunStatus } from '@/stores/specialPreRun';
...
export const SpecialPreRunSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  const [form] = Form.useForm();
  const { filters, setFilters } = useSpecialPreRunStore();
  const [collapsed, setCollapsed] = useState(true);

  const statusOptions: { value: PreRunStatus; label: string }[] = [
    '未下单','待开始','进行中','已完成','异常'
  ].map(s => ({ value: s as PreRunStatus, label: s }));
  ...
  return (
    <Card
      ...
      extra={
        <Button type="link" icon={collapsed ? <DownOutlined /> : <UpOutlined />} onClick={() => setCollapsed(!collapsed)}>
          {collapsed ? '展开' : '收起'}
        </Button>
      }
    >
        ...
        <Row gutter={16}>
          <Col xs={24} sm={12} md={6} lg={6} xl={6}>
            <Form.Item label="样本编号" name="sampleNos">
              <Input placeholder="多个样本号用英文逗号分隔" />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} md={6} lg={6} xl={6}>
            <Form.Item label="产品名称" name="productIds">
              <Input placeholder="请选择产品或输入关键字" />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} md={6} lg={6} xl={6}>
            <Form.Item label="任务状态" name="statuses">
              <Select mode="multiple" placeholder="请选择任务状态">
