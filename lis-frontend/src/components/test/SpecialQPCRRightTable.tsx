...
export const SpecialQPCRRightTable: React.FC = () => {
  const { filteredItems } = useSpecialQPCRStore();
  const columns: ColumnsType<any> = [
    ...
    { title: 'Cq值', dataIndex: 'cq', key: 'cq', width: 120 },
    { title: '标准曲线', dataIndex: 'curve', key: 'curve', width: 180 }
  ];
  const totalWidth = columns.reduce((sum, col) => sum + (typeof col.width === 'number' ? col.width : 120), 0);
  return (
    <Table rowKey="id" columns={columns} dataSource={filteredItems} scroll={{ x: totalWidth }} pagination={false} />
  );
};
