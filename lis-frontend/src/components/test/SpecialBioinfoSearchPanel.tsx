...
import { useSpecialBioinfoStore, BioStatus, AuditStatus } from '@/stores/specialBioinfo';
const { RangePicker } = DatePicker; const { Option } = Select;
interface Props { onSearch: () => void; onReset: () => void; }
export const SpecialBioinfoSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  const [form] = Form.useForm(); const { filters, setFilters } = useSpecialBioinfoStore(); const [collapsed, setCollapsed] = useState(true);
  const statusOptions: { value: BioStatus; label: string }[] = [ '待分析','进行中','已完成','失败' ].map(s => ({ value: s as BioStatus, label: s }));
  const auditOptions: { value: AuditStatus; label: string }[] = [ '待审核','已审核','不通过' ].map(s => ({ value: s as AuditStatus, label: s }));
  useEffect(() => { form.setFieldsValue({ sampleNos: filters.sampleNos?.join(', ') || '', productIds: filters.productIds?.join(', ') || '', statuses: filters.statuses || ['待分析','进行中','失败'], auditStatuses: filters.auditStatuses || undefined }); }, [filters, form]);
    ...
    <Card title="查询条件" size="small" styles={{ body: { padding: 16 } }} extra={<Button type="link" icon={collapsed ? <DownOutlined /> : <UpOutlined />} onClick={() => setCollapsed(!collapsed)}>{collapsed ? '展开' : '收起'}</Button>}>
      <Form form={form} layout="vertical">
        <Row gutter={16}>
          ...
          <Col xs={24} sm={12} md={6} lg={6} xl={6}><Form.Item label="分析状态" name="statuses"><Select mode="multiple" placeholder="请选择分析状态">{statusOptions.map(s => (<Option key={s.value} value={s.value}>{s.label}</Option>))}</Select></Form.Item></Col>
