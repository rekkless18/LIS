...
export const ProductActionBar: React.FC<Props> = ({ onQuery, onReset }) => {
  ...
  const [testItemOptions, setTestItemOptions] = useState<{ id: string, item_code: string, item_name: string }[]>([]);

  /**
   * 功能描述：加载启用的检测项（供产品弹窗选择）
   * 参数说明：无
   * 返回值类型及用途：无；内部更新 testItemOptions 状态
   */
  const loadEnabledTestItems = async () => {
    const API_BASE = (import.meta as any).env?.VITE_API_BASE || 'http://localhost:3001/api';
    const resp = await fetch(`${API_BASE}/test-items/enabled`);
    const json = await resp.json();
    setTestItemOptions(json.data || []);
  };

  const handleOpenCreate = () => { form.resetFields(); setOpenCreate(true); };
  const handleCreateSubmit = () => {
    form.validateFields().then(values => {
      createItem({ productCode: values.productCode, productName: values.productName, productType: values.productType as ProductType, status: values.status as EnableStatus, testItems: [], testItemIds: (values.testItems || []) });
      setOpenCreate(false);
  ...
  const handleOpenEdit = () => {
    ...
    form.setFieldsValue({ productCode: target.productCode, productName: target.productName, productType: target.productType, status: target.status, testItems: target.testItemIds });
    setOpenEdit(true);
  };
  const handleEditSubmit = () => {
    const id = selectedRowKeys[0] as string;
    form.validateFields().then(values => {
      editItem(id, { productCode: values.productCode, productName: values.productName, productType: values.productType as ProductType, status: values.status as EnableStatus, testItems: [], testItemIds: (values.testItems || []) });
      setOpenEdit(false);
  ...
  return (
    <div className="flex justify-between items-center py-2">
      ...
      <Modal title="新建产品" open={openCreate} onOk={handleCreateSubmit} onCancel={() => setOpenCreate(false)} okText="提交" cancelText="关闭" afterOpenChange={(opened) => { if (opened) loadEnabledTestItems(); }}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="检测项" name="testItems"><Select mode="multiple" placeholder="请选择检测项">{testItemOptions.map(opt => (<Option key={opt.id} value={opt.id}>{opt.item_name}</Option>))}</Select></Form.Item>
      ...
      <Modal title="编辑产品" open={openEdit} onOk={handleEditSubmit} onCancel={() => setOpenEdit(false)} okText="提交" cancelText="关闭" afterOpenChange={(opened) => { if (opened) loadEnabledTestItems(); }}>
        <Form form={form} layout="vertical">
          <Form.Item label="产品编码" name="productCode" rules={[{ required: true, message: '请输入产品编码' }]}><Input /></Form.Item>
          <Form.Item label="产品名称" name="productName" rules={[{ required: true, message: '请输入产品名称' }]}><Input /></Form.Item>
          <Form.Item label="产品类型" name="productType" rules={[{ required: true, message: '请选择产品类型' }]}><Select placeholder="请选择">{(['普检产品','特检产品','质谱产品','研发产品','其他产品'] as ProductType[]).map(t => (<Option key={t} value={t}>{t}</Option>))}</Select></Form.Item>
