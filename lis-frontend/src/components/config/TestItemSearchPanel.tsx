...
export const TestItemSearchPanel: React.FC<Props> = ({ onSearch, onReset }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      itemTypes: filters.itemTypes || typeOptions,
      judgeTypes: filters.judgeTypes || judgeOptions,
  ...
  const handleSearch = () => {
    form.validateFields().then(values => {
      const next = {
        ...
        itemTypes: values.itemTypes || undefined,
        judgeTypes: values.judgeTypes || undefined,
    ...
    <Card title="查询条件" size="small" styles={{ body: { padding: 16 } }} extra={<Button type="link" icon={collapsed ? <DownOutlined /> : <UpOutlined />} onClick={() => setCollapsed(!collapsed)}>{collapsed ? '展开' : '收起'}</Button>}>
      <Form form={form} layout="vertical">
        <Row gutter={16}>
          ...
          <Col xs={24} sm={12} md={6}><Form.Item label="检测项类型" name="itemTypes"><Select mode="multiple" placeholder="请选择检测项类型">{typeOptions.map(t => (<Option key={t} value={t}>{t}</Option>))}</Select></Form.Item></Col>
          <Col xs={24} sm={12} md={6}><Form.Item label="结果判断类型" name="judgeTypes"><Select mode="multiple" placeholder="请选择结果判断类型">{judgeOptions.map(j => (<Option key={j} value={j}>{j}</Option>))}</Select></Form.Item></Col>
        </Row>
        {!collapsed && (
          <>
            <Row gutter={16}>
